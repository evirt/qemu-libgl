...
libGL.so.1.2: client_stub.c opengl_client.c glgetv_cst.h opengl_func.h opengl_utils.h mesa_gl.h mesa_glext.h mesa_glx.h mesa_glxext.h client_gl.o log.o opengl_utils.o gl_tables.o client_glx.o range_alloc.o
	rm -f server_stub.c
	$(CC) -fPIC $(GL_CFLAGS) opengl_client.c -shared -o libGL.so -lX11 -lXfixes -lm -lXext -L$(D)/usr/X11R6/lib -lpthread -I. client_gl.o log.o opengl_utils.o gl_tables.o client_glx.o range_alloc.o
...
clean:
	rm -f client_stub.c gl_func.h glgetv_cst.h parse_gl_h parse_mesa_get_c *.o gl_func_tabs.h libGL.so
